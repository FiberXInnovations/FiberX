...
export default {
    data() { 
        return {
            content: this.$root.content_data.Scroller,
        }
    },
    methods: {
        isAtBottom() { 
...
<template>
    <section id="Scroller" class="absolute bottom-24 right-10 flex flex-col gap-4 z-20">
        <div v-if="$root.y > 0" @click="scrollPageUp" class="rounded-full w-[50px] h-[50px] bg-[#21201D] animate-bounce p-2 cursor-pointer scroller">
            <div class="w-full h-full items-center flex justify-center text-white" v-html="content.scroll_up.icon"> 
            </div>
        </div>
        <div  @click="scrollPageDown" class="rounded-full w-[50px] h-[50px] bg-[#21201D] animate-bounce p-2 cursor-pointer scroller">
